...
class Canopus:
    ...
    async def start(self):
        ...
        try:
            while True:
                audio_data = await self.process_audio_queue()
                if audio_data:
                    text = self.stt.transcribe(audio_data)
                    
                    # Wake word detection
                    if "hey canopus" in text.lower():
                        self.tts.speak("Yes, I'm listening")
                        response = await self.process_command(text)
                        self.tts.speak(response)
                    elif "show history" in text.lower():
                        last_commands = self.command_history.get_last_commands()
                        self.display_command_history(last_commands)
